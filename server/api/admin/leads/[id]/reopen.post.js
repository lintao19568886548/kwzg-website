...
import { reopenLead } from '../../../../utils/lead-outcomes.js'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    await enforceAdminApiRateLimit(db, config, event, session, 'lead-reopen', 20)
    const body = await readStrictJsonBody(event, { limit: 2048 }); assertExactKeys(body, ['version', 'note'])
    return { ok: true, ...await reopenLead(db, getRouterParam(event, 'id'), body, { admin: session.admin, requestId: event.context.requestId, sourceIp: getClientAddress(event, config.trustedProxyAddresses) }) }
