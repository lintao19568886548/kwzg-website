...
- Stage 1 审计时 AI 招商匹配按当时信息保留在独立规划区；产品方已于 2026-08-21 修正其状态为“现有能力”。由于当前截图集仍未包含独立 AI 页面，其具体字段、按钮、结果和操作界面继续不进入真实系统界面组。
...
- 佛山九江带 `ndocn.com` 水印的旧原图未进入公开目录；官网现使用用户后来补充并完成核验的无水印图片。
- 当前公开园区图：同富 1 张、佛山乐从 2 张、佛山九江 1 张、深圳坑梓 1 张、高埗同兴 1 张、新塘西州 1 张。
- 2026-08-21 用户明确将馆峰建筑照片归属同富园区；该图片已从原错误的佛山乐从归类中移除，官网不存在同图多园区展示。
- 2026-08-21 用户重新提供的佛山九江园区图片未发现水印；公开版为 800×500 WebP、EXIF 条目为 0，旧 `ndocn.com` 水印图未进入公开目录。

> 后续状态：Stage 2 最新案例门禁重新将佛山九江列为不可公开，相关派生图已从发布目录删除。本段仅保留 Stage 1 当时的验收记录。
