...
- 完整产品能力统一维护在 `app/data/product-capabilities.js`：206 项能力的公开 `availability` 均为 `available`，并用 `deliveryMode` 区分标准软件、项目配置、接口实施和云端私有化部署。首页按七大能力域、三条业务闭环、九个系统模块和六类岗位组织能力，缺少独立页面截图只影响视觉证据，不影响产品完成状态。
